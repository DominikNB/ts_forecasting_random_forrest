...
def create_all_seasonality_dummys_direct(base_frequency: str, start_time=False, length=False, date_vector=False):

    if base_frequency == "dayly":
        indicator_monthly = np.zeros(len(date_vector))
        for i in range(1, 13):
            indicator_monthly[date_vector.dt.month == i] = i

        indicator_quarterly = np.zeros(len(date_vector))
        for i in range(1, 13):
            if i == 1 or i == 2 or i == 3:
                indicator_quarterly[date_vector.dt.month == i] = 1
            if i == 4 or i == 5 or i == 6:
                indicator_quarterly[date_vector.dt.month == i] = 2
            if i == 7 or i == 8 or i == 9:
                indicator_quarterly[date_vector.dt.month == i] = 3
            if i == 10 or i == 11 or i == 12:
                indicator_quarterly[date_vector.dt.month == i] = 4

        indicator_dayly = np.zeros(len(date_vector))
        for i in range(len(date_vector)):
                indicator_dayly[i] = date_vector[i].weekday()+1

        indicator_numerator = np.arange(len(date_vector)) + 1

        indicators = np.vstack((indicator_dayly, indicator_quarterly, indicator_monthly, indicator_numerator))
        print('test')
        ...
        qrt = np.ones(3)
        indicator_quarterly = np.hstack((qrt, qrt*2, qrt*3,qrt*4))
        indicator_quarterly = np.tile(indicator_quarterly, 200)

        indicator_monthly = np.arange(12)+1
        indicator_monthly = np.matrix(np.tile(indicator_monthly, 200))

        business_cycle_3 = np.ones(12)
        indicator_business_cycle_3 = np.hstack((business_cycle_3, business_cycle_3 * 2, business_cycle_3 * 3))
        indicator_business_cycle_3 = np.tile(indicator_business_cycle_3, 200)
        indicator_business_cycle_3 = indicator_business_cycle_3[0:indicator_monthly.shape[1]]

        business_cycle_5 = np.ones(12)
        indicator_business_cycle_5 = np.hstack((business_cycle_5, business_cycle_5*2, business_cycle_5*3, business_cycle_5*4, business_cycle_5*5))
        indicator_business_cycle_5 = np.tile(indicator_business_cycle_5, 200)
        indicator_business_cycle_5 = indicator_business_cycle_5[0:indicator_monthly.shape[1]]

        business_cycle_7 = np.ones(12)
        indicator_business_cycle_7 = np.hstack((business_cycle_7, business_cycle_7*2, business_cycle_7*3, business_cycle_7*4, business_cycle_7*5, business_cycle_7*6, business_cycle_7*7))
        indicator_business_cycle_7 = np.tile(indicator_business_cycle_7, 200)
        indicator_business_cycle_7 = indicator_business_cycle_7[0:indicator_monthly.shape[1]]

        business_cycle_10 = np.ones(12)
        indicator_business_cycle_10 = np.hstack((business_cycle_10, business_cycle_10 * 2, business_cycle_10 * 3,
                                                business_cycle_10 * 4, business_cycle_10 * 5, business_cycle_10 * 6,
                                                business_cycle_10 * 7, business_cycle_10 * 8, business_cycle_10 * 9, business_cycle_10 * 10))
        indicator_business_cycle_10 = np.tile(indicator_business_cycle_10, 200)
        indicator_business_cycle_10 = indicator_business_cycle_10[0:indicator_monthly.shape[1]]

        indicator_numerator = np.arange(indicator_monthly.shape[1]) + 1
        indicators = np.vstack((indicator_quarterly, indicator_monthly, indicator_business_cycle_3, indicator_business_cycle_5, indicator_business_cycle_7, indicator_business_cycle_10, indicator_numerator))
